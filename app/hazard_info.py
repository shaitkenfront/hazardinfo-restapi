...
# 大規模盛土造成地
ENABLE_LARGE_FILL_LAND = os.environ.get('ENABLE_LARGE_FILL_LAND', 'false').lower() == 'true'
...
def _get_max_info_from_tile(lat: float, lon: float, tile_url_template: str, tile_zoom: int, color_map: dict, no_risk_description: str, high_precision: bool = False) -> dict:
    """
    ...
    """
    if high_precision:
        return _get_max_info_from_tile_high_precision(lat, lon, tile_url_template, tile_zoom, color_map, no_risk_description)
    else:
        return _get_max_info_from_tile_8_directions(lat, lon, tile_url_template, tile_zoom, color_map, no_risk_description)

def _get_max_info_from_tile_8_directions(lat: float, lon: float, tile_url_template: str, tile_zoom: int, color_map: dict, no_risk_description: str) -> dict:
    """
    中心点とその周囲8方位のピクセルを調査する高速版
    """
    # 中心点の座標を取得
    zoom, x_tile, y_tile, center_px, center_py = latlon_to_gsi_tile_pixel(lat, lon, tile_zoom)
...
def _get_max_info_from_tile_high_precision(lat: float, lon: float, tile_url_template: str, tile_zoom: int, color_map: dict, no_risk_description: str) -> dict:
    """
...
def get_tsunami_inundation_info_from_gsi_tile(lat: float, lon: float, high_precision: bool = False) -> dict:
    """
    国土地理院の津波浸水想定タイルから、中心点と半径100m以内の最大浸水深を取得する。
    """
    return _get_max_info_from_tile(lat, lon, TSUNAMI_TILE_URL, TSUNAMI_TILE_ZOOM, TSUNAMI_COLOR_MAP, "浸水想定なし", high_precision)

def get_high_tide_inundation_info_from_gsi_tile(lat: float, lon: float, high_precision: bool = False) -> dict:
    """
    国土地理院の高潮浸水想定タイルから、中心点と半径100m以内の最大浸水深を取得する。
    """
    return _get_max_info_from_tile(lat, lon, HIGH_TIDE_TILE_URL, HIGH_TIDE_TILE_ZOOM, HIGH_TIDE_COLOR_MAP, "浸水想定なし", high_precision)
...
def get_jshis_info(lat: float, lon: float, high_precision: bool = False) -> dict[str, str]:
    """
...
def get_inundation_depth_from_gsi_tile(lat: float, lon: float, high_precision: bool = False) -> dict:
    """
...
def get_steep_slope_info_from_gsi_tile(lat: float, lon: float, high_precision: bool = False) -> dict:
    """
    国土地理院の急傾斜地の崩壊警戒区域タイルから情報を取得する。
    """
    return _get_max_info_from_tile(lat, lon, STEEP_SLOPE_TILE_URL, STEEP_SLOPE_TILE_ZOOM, STEEP_SLOPE_COLOR_MAP, "該当なし", high_precision)

def get_debris_flow_info_from_gsi_tile(lat: float, lon: float, high_precision: bool = False) -> dict:
    """
    国土地理院の土石流警戒区域タイルから情報を取得する。
    """
    return _get_max_info_from_tile(lat, lon, DEBRIS_FLOW_TILE_URL, DEBRIS_FLOW_TILE_ZOOM, DEBRIS_FLOW_COLOR_MAP, "該当なし", high_precision)

def get_landslide_info_from_gsi_tile(lat: float, lon: float, high_precision: bool = False) -> dict:
    """
    国土地理院の地すべり警戒区域タイルから情報を取得する。
    """
    return _get_max_info_from_tile(lat, lon, LANDSLIDE_TILE_URL, LANDSLIDE_TILE_ZOOM, LANDSLIDE_COLOR_MAP, "該当なし", high_precision)
    
def get_large_scale_filled_land_info_from_geojson(lat: float, lon: float, high_precision: bool = False) -> dict:
    """
...
def get_selective_hazard_info(lat: float, lon: float, hazard_types: list[str] = None, high_precision: bool = False) -> dict[str, str]:
    """
    指定されたハザード情報のみを取得する。
    ...
    if 'tsunami' in hazard_types:
        tsunami_info = get_tsunami_inundation_info_from_gsi_tile(lat, lon, high_precision)
    ...
    if 'high_tide' in hazard_types:
        hightide_info = get_high_tide_inundation_info_from_gsi_tile(lat, lon, high_precision)
    ...
    if 'large_fill_land' in hazard_types:
        if ENABLE_LARGE_FILL_LAND:
            large_fill_land_info = get_large_scale_filled_land_info_from_geojson(lat, lon, high_precision)
...
def get_all_hazard_info(lat: float, lon: float, high_precision: bool = False) -> dict[str, str]:
    """
